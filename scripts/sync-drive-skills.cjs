...
// ── --sync-lanes (added by Claude Desktop chat session · 2026-07-01 · Rule 73 candidate) ──────────
// CONTEXT_SKILL.md's TERMINAL LANES section was hand-retyped from CLAUDE.md and drifted: it showed T3
// owning all of src/hooks/ when CLAUDE.md actually carves out 3 specific files for T3 and leaves the
// rest of src/hooks/ to T2, and it omitted T1's grant of src/App.jsx + src/utils/ entirely. This
// extracts the block VERBATIM from CLAUDE.md instead of re-paraphrasing it, so the only way this
// output can change is if CLAUDE.md's own grant changes. Purely additive: does not touch any other
// command, any other Drive doc, or any existing function. Idempotent (safe to run twice).
const CLAUDE_MD_PATH = path.join(__dirname, '../.claude/CLAUDE.md');
const CONTEXT_SKILL_PATH = path.join(SKILLS_DIR, 'CONTEXT_SKILL.md');

function extractTerminalLanes(claudeMdText) {
  const startMarker = 'TERMINAL LANES:';
  const startIdx = claudeMdText.indexOf(startMarker);
  if (startIdx === -1) {
    throw new Error('TERMINAL LANES: block not found in CLAUDE.md — source format changed, fix this extractor before trusting CONTEXT_SKILL.md');
  }
  const afterStart = claudeMdText.slice(startIdx + startMarker.length);
  const endMatch = afterStart.match(/\n\n[A-Z][A-Z0-9 _/()-]*:/);
  const block = endMatch ? afterStart.slice(0, endMatch.index) : afterStart.split('\n\n')[0];
  return (startMarker + block).trim();
}

async function syncLanesFromClaudeMd() {
  if (!fs.existsSync(CLAUDE_MD_PATH)) { console.error(`❌ CLAUDE.md not found: ${CLAUDE_MD_PATH}`); process.exit(1); }
  if (!fs.existsSync(CONTEXT_SKILL_PATH)) { console.error(`❌ CONTEXT_SKILL.md not found: ${CONTEXT_SKILL_PATH}`); process.exit(1); }
  const claudeMd = fs.readFileSync(CLAUDE_MD_PATH, 'utf8');
  const extracted = extractTerminalLanes(claudeMd);
  const replacement = `## TERMINAL LANES (auto-extracted verbatim from CLAUDE.md by --sync-lanes, do not hand-edit this section, edit CLAUDE.md instead)\n${extracted}`;

  const contextMd = fs.readFileSync(CONTEXT_SKILL_PATH, 'utf8');
  const sectionRe = /## TERMINAL LANES[\s\S]*?(?=\n## )/;
  if (!sectionRe.test(contextMd)) { console.error('❌ "## TERMINAL LANES" section not found in CONTEXT_SKILL.md — fix the section header before relying on this'); process.exit(1); }
  const updated = contextMd.replace(sectionRe, replacement + '\n');

  if (updated === contextMd) { console.log('✅ Lanes already in sync · no change needed'); return; }
  fs.writeFileSync(CONTEXT_SKILL_PATH, updated, 'utf8');
  await writeDocContent(FILE_IDS.context, updated, 'CONTEXT_SKILL.md (lanes auto-synced from CLAUDE.md)');
  console.log('✅ Lane grant re-extracted from CLAUDE.md → CONTEXT_SKILL.md (local file + Drive both updated)');
}
...
async function main() {
  ...
  if (cmd === '--test') await testConnection();
  ...
  else if (cmd === '--sync-lanes') await syncLanesFromClaudeMd();
    ...
    console.log('--test · --all · --skill <name> · --log-flaw · --log-session · --log-terminal-review · --list-card-art · --sync-lanes · --validate-manifest');
