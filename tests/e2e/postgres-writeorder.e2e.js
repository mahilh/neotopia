...
//
// ✅ CI: live · nightly (e2e-live-nightly.yml, T2 S48). It was the LAST spec in this repo running in no
// workflow at all, and it carried an orphan declaration (the RUNS-NOWHERE marker, colon deliberately
// omitted here so this sentence cannot satisfy the gate that reads for it) from S43 until that. Proven green
// BEFORE it was wired, never after (Rule 79a · the S37 spec that was wired blind had a stale test.fail()
// and arrived red): 1 passed in 6.4s at HEAD 4008f46, and the measurement reproduced exactly · wrote
// B(seq 2, seat 1) then A(seq 1, seat 0), the row holds A, current_seat 0.
// The declaration was deleted only AFTER the workflow line reached origin, and the order was not fussiness:
// preconditions' orphan gate reads .github/ as it exists at the COMMIT boundary, so removing it first
// would have reddened the merge gate for all three lanes (Rule 67 · gate what is true WHERE the gate runs).
// Run locally:  npx playwright test tests/e2e/postgres-writeorder.e2e.js
//
