...
import { deadlockStatePatch, countLegalPlacements } from './deadlockFixture'
...
// fillRegion / the factory geometry / the legal-placement count now live in ./deadlockFixture.js
// (T2 S47). T3 rebuilt this board for their browser gate and hit my first-draft bug exactly · 2 legal
// placements in a state meant to have none. Two lanes, same bug, same 2, so it is one module now.
...
function seedDeadlock({ tiles = 3, deck = [], offer = [] } = {}) {
  ...
  useGameStore.setState(deadlockStatePatch(store().regions, { tiles, deck, offer }), false)
}

const legalPlacementCount = () => countLegalPlacements(store(), store().getValidPlacements)
