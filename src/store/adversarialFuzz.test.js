...
const canDraw = () => store().theOffer.length > 0 || store().deck.length > 0
...
function playWithPolicy(policyName, seed, { gate = 't1', turnCap = 400, mode } = {}) {
  ...
  let tilesAtTrigger = null, maxHand = 0
  while (store().phase === 'playing' && turns < turnCap) {
    ...
    if (!uiAllowsEndTurn(gate)) return { outcome: 'hung', turns, state: snapshot() }
    store().endTurn()
    turns++
    const st = store()
  ...
  return { outcome: store().phase === 'scoring' ? 'scoring' : 'capped', turns, tilesAtTrigger, maxHand, state: snapshot() }
}
